...
export default function DropDownButtons() {
  ...
  const [showSubcategoryOptions, setShowSubcategoryOptions] = useState(
  ...
  const handleCategoryClick = (index) => {
    ...
    newSelectedCategory[index] =
      newSelectedCategory[index] === index ? null : index;
  ...
  const handleSubcategoryClick = (categoryIndex, subcategoryIndex) => {
    const newSelectedSubcategory = [...selectedSubcategory];
    newSelectedSubcategory[categoryIndex] =
      newSelectedSubcategory[categoryIndex] === subcategoryIndex
        ? null
        : subcategoryIndex;
    setSelectedSubcategory(newSelectedSubcategory);
  ...
  return (
    <>
      {categories.map((category, index) => (
        ...
        <div
          key={index}
          style={{ position: "relative", zIndex: categories.length - index }}
        >
          <div className="relative inline-block">
            <div className="flex-none p-2">
              <button
                onClick={() => handleCategoryClick(index)}
                className="w-48 px-2 py-2 text-gray-700 lg:text-base text-xs bg-white border-2 border-white rounded-md shadow focus:outline-none focus:border-blue-600"
                ...
                <span
                  className={
                    showSubcategoryOptions[index]
                      ? "w-6 h-6 stroke-current transform rotate-180"
              ...
              {showSubcategoryOptions[index] && (
                <div
                  className="absolute left-1.75 mt-2 w-48 bg-white rounded-lg shadow-xl"
                  style={{ zIndex: categories.length }}
                >
                  <div className="py-1">
                    {subcategories[index].map((subcategory, subIndex) => (
                      <button
                        ...
                        className={`w-48 px-4 py-2 text-left text-sm ${
                          selectedSubcategory[index] === subIndex
                  ...
                  {selectedSubcategory[index] !== null && (
                    <div
                      className="absolute left-48 top-0 ml-2 mt-0 w-48 bg-white rounded-lg shadow-xl"
                      style={{ zIndex: categories.length }}
                    >
                      <div className="py-1">
                        {items[index][selectedSubcategory[index]].map(
                          (item, itemIndex) => (
                            <button
                              key={itemIndex}
                              onClick={() =>
